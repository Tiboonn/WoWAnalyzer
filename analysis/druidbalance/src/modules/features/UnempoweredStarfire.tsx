...
class UnempoweredStarfire extends Analyzer {
  get badCastsPerMinute() {
    return ((this.badCasts - (this.empowermentAppliedCount * UNEMPOWERED_CASTS_NEEDED_FOR_EMPOWERMENT)) / (this.owner.fightDuration / 1000)) * 60;
  }
  ...
  onApplyBuff(event: ApplyBuffEvent) {
    this.empowermentAppliedCount += 1;
  }
  ...
  suggestions(when: When) {
    when(this.suggestionThresholds).addSuggestion((suggest, actual, recommended) => suggest(<>You cast {this.badCasts} unempowered outside the {UNEMPOWERED_CASTS_NEEDED_FOR_EMPOWERMENT} needed to get Eclipse and non instant cast <SpellLink id={SPELLS.STARFIRE.id} /> that hit less than {TARGETS_FOR_GOOD_CAST} targets.</>)
